...
t('the deploy writes standalone sandbox configs, not ones that require production', function () use ($root) {
    $sh = file_get_contents($root . '/deploy.sh');
    hasnt('$c = require "/home/protected/lib/config.php"', $sh,
        'no sandbox config is generated by inheriting production any more');
    has("'session_name' => 'SCSESS_", $sh, 'each generated config pins its own session name');
    has("'users'        => ['\$name' => '\$pw']", $sh, 'and carries its own bootstrap account');
    has('random_bytes', $sh, 'whose password is generated, not a literal');
    // An existing inheriting config must be reported rather than silently rewritten:
    // replacing it changes who can log in to a live instance.
    has('still INHERITS', $sh, 'an older inheriting config is flagged');
    hasnt('rm -rf', $sh, 'and nothing in the deploy deletes recursively');
});
...
t('only "Calendar" is a permanent folder now; "Reminders" can be deleted', function () {
    ...
    req('POST', '/reminders/', ['csrf' => csrf($jar), 'action' => 'add', 'view' => FOLDER_CALENDAR,
